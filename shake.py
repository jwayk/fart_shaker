...
class Shaker:
  ...
  def shake(self, source_name, trigger=False):
    ...
    frame = int(((time.time() - self.start_time) / self.anim.duration) * self.anim.resolution)
    if frame >= len(self.anim.function):
      self.start_time = None
      return 

    angle = self.anim.function[frame]
...
def script_load(settings):
  global hotkey_id
  hotkey_id = obs.obs_hotkey_register_frontend(script_path(), "Fart Shake", on_shake_hotkey)
